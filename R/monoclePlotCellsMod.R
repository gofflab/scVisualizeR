#Edits from og monocle3 function are marked by (JS)

#TODO Add Roxygen stuff
...
#'   of the cell_size
#' @param outline_cells Whether to outline (underplot) cells. Default: FALSE
#' @param outline_cell_size The size of the point for each underplotted cell
#' @param outline_cell_color The color of each underplotted cell
...
plot_cells_mod <- function(cds,
                       ...
                       cell_size=0.35,
                       outline_cells = F, # (JS)
                       outline_cell_size = 0.45, # (JS)
                       outline_cell_color = "grey80", # (JS)
  ...
  #Commented out by (JS) so other custom names reduced dimensions can be plotted
  ...
  if (!is.null(markers_exprs) && nrow(markers_exprs) > 0){
    ...
    #Added by (JS) to be compatible with facet_wrap()
    ...
    if(outline_cells){
      ...
      #Added by (JS)
      #underplot. Use data_null so that points won't be separated by facets
      g <- g + geom_point(data = data_null, size=I(outline_cell_size),
                            stroke = I(cell_stroke), na.rm = TRUE,
                            alpha = alpha, color = outline_cell_color)
